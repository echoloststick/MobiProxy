...
// *itálico* -> <i>itálico</i>. Roda DEPOIS do applyBold -- nesse ponto os
// pares ** já viraram <b>, então qualquer * que sobrou é itálico de
// verdade, não sobra de negrito mal casado.
function applyItalic(text) {
	return text.replace(/\*([^*\n]+?)\*/g, '<i>$1</i>');
}

// `código inline` -> <font face="Code">código inline</font>, sem colorir
// (colorize só faz sentido pro bloco ``` inteiro, não pra um trechinho
// solto no meio da frase). Roda ANTES de bold/italic pra um `*` dentro do
// crase não ser lido como itálico por engano.
function applyInlineCode(text) {
	return text.replace(/`([^`\n]+?)`/g, '<font face="Code">$1</font>');
}

// Um trecho de texto NORMAL (fora de bloco de código ```): escapa pra não
// quebrar o RichText e converte o markdown -- código inline primeiro,
// depois negrito, depois itálico.
function formatPlainSegment(text) {
	let result = escapeRichText(text);
