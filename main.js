...
//                     (conteudo bruto). Rota publica (sem exigir x-api-key do caller, igual
//                     /get) -- a API key do Open Cloud, se usada, mora so na env var
//                     OPEN_CLOUD_API_KEY do proxy, nunca vem do Script do jogo. Ela sempre "age
//                     como" a conta que a gerou (nao tem escopo que vire outro Player), entao
//                     nao ha motivo pro jogo carregar essa credencial. O que a rota resolve de
//                     verdade: o InsertService:LoadAsset() do lado do jogo sempre devolve
//                     Source vazio pra asset que nao pertence ao jogo/grupo atual (protecao
//                     anti-copia da Roblox) -- aqui o conteudo (quando publico/gratuito) vem
//                     completo, o que ja cobre o caso comum de asset da pagina inicial da
//                     Store. Asset privado de OUTRO Player continua fora de alcance, com ou
//                     sem key (precisaria de OAuth com consentimento do usuario, que e um
//                     fluxo bem maior -- nao implementado aqui).
...
app.get('/asset', async (req, res) => {
	...
	// A API key (se configurada) mora só aqui, como variavel de ambiente do
	// Render -- nao vem mais do caller. Ela nunca ia liberar asset privado de
	// OUTRO Player mesmo (key sempre "age como" a conta que a gerou, nao tem
	// escopo que vire outro usuario -- ver conversa anterior), entao nao
	// fazia sentido o Script do jogo carregar/expor essa credencial. Sem a
	// env var configurada, a metadata do Open Cloud so vem vazia (best-
	// effort, ver abaixo) -- o conteudo publico via assetdelivery continua
	// funcionando igual, ele nunca usou essa chave mesmo.
	const apiKey = process.env.OPEN_CLOUD_API_KEY || null;

	// Metadata via Open Cloud -- best-effort: se o path/formato da resposta
	// mudou de versao (nao confirmado aqui), ou se a env var nao tiver
	// configurada, so seguimos sem assetType, o Lua cai pro que o client
	// mandou.
	let meta = null;
	if (apiKey) {
		try {
	...
	// Conteudo bruto via Asset Delivery -- e onde o Source de Script vem
	// preenchido de verdade (diferente do InsertService em jogo ao vivo).
	// NAO manda x-api-key aqui: assetdelivery.roblox.com e um endpoint
	// legado, nao Open Cloud, e nao reconhece esse header -- so funciona (ou
	// nao) pra qualquer um igual, tenha chave ou nao. Asset privado/pago que
	// a conta dona da API key nao tem direito continua bloqueado do mesmo
	// jeito -- nao existe atalho de credencial pra isso, e o modelo de
	// permissao da Roblox mesmo.
	let scripts = [];
	...
	try {
		...
		if (contentRes.ok) {
			...
			hasContent = buf.length > 0;
			const head = buf.slice(0, 8).toString('latin1');

			if (head === '<roblox!') {
				scripts = extractScriptsFromBinary(buf);
			} else if (head.startsWith('<roblox')) {
				scripts = extractScriptsFromXML(buf.toString('utf8'));
			}
		} else {
