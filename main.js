...
//                     Sem key, ou se isso falhar: cai pro endpoint legado publico
//                     (assetdelivery.roblox.com), que so serve asset publico/gratuito de
//                     qualquer forma.
...
function xmlPropToValue(propNode) {
	...
	switch (tag) {
		...
		case 'Color3':
			return { r: num(getChildText('R')), g: num(getChildText('G')), b: num(getChildText('B')) };
		case 'UDim':
...
function readBinPropArray(typeId, buf, count) {
	switch (typeId) {
		...
		case 0x0c: { // Color3
			...
			const out = new Array(count);
			for (let i = 0; i < count; i++) out[i] = { r: r[i], g: g[i], b: b[i] };
			return out;
		}
		case 0x0d: { // Vector2
			let off = 0;
		...
		case 0x1a: { // Color3uint8
			const out = new Array(count);
			for (let i = 0; i < count; i++) {
				out[i] = { r: buf.readUInt8(i) / 255, g: buf.readUInt8(count + i) / 255, b: buf.readUInt8(2 * count + i) / 255 };
			}
			return out;
		}
...
app.get('/asset', async (req, res) => {
	...
	// Conteudo bruto. Com API key: tenta primeiro o endpoint AUTENTICADO do
	// Open Cloud (asset-delivery-api) -- esse sim reconhece x-api-key de
	// verdade e e o que pode liberar asset privado do dono da key (escopo
	// "legacy-assets:manage"). Sem key, ou se o autenticado falhar: cai pro
	// endpoint legado publico (assetdelivery.roblox.com), que so funciona
	// pra asset publico/gratuito de qualquer forma.
	let buf = null;
	if (apiKey) {
		try {
