...
// Versão pro Script Editor: devolve o código colorido inteiro (igual colorizeLua -- um token
// multi-linha, tipo comentário/string longa, vira UM <font> só que atravessa as linhas, o que o
// RichText aceita numa exibição contínua), o código colorido QUEBRADO POR LINHA (cada linha com
// tags próprias e balanceadas, pra dar pra re-renderizar/recolorir uma linha sozinha sem herdar
// tag aberta da linha anterior) e a lista de erros de sintaxe encontrados.
function colorizeWithErrors(code) {
	...
	for (const t of tokens) {
		const color = colorOf(t);
		colored += color ? wrap(t.text, color) : escapeRichText(t.text);
